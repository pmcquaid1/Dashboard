...
def dashboard(request):
    ...
    if user.is_superuser:
        visible_departments = DEPARTMENT_CARD_CONFIG.keys()
    else:
        visible_departments = user_groups
    ...
    for dept in visible_departments:
        ...
        if config:
            ...
            # Optional fallback for unconfigured departments
            cards.append({
                "title": dept,
    ...
    print("Groups:", list(user_groups))
    print("Visible departments:", list(visible_departments))
    print("Cards:", [card["title"] for card in cards])
...
@login_required
def c_f(request): return render(request, 'c_f.html')

@login_required
def revops(request): return render(request, 'revops.html')

@login_required
def documentation(request): return render(request, 'documentation.html')

@login_required
def warehouse(request): return render(request, 'warehouse.html')
...
def login_user(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            elif not (user_obj.is_superuser or user_obj.is_staff):
                reason = "User lacks access privileges"
            else:
                # Use email or username dynamically
        ...
        if user:
            ...
            # Safe increment for LocMemCache
            if attempts == 0:
